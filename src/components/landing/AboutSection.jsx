        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <motion.div variants={container} initial="hidden" animate={isInView ? "show" : "hidden"}>
            <motion.div variants={item} className="mb-4">
              <span className="inline-block px-4 py-1.5 rounded-full bg-emerald-100 text-emerald-700 text-sm font-semibold tracking-wide uppercase">About Zahoor</span>
            </motion.div>
            <motion.h2 variants={item} className="text-4xl md:text-5xl font-black text-emerald-900 mb-6 leading-tight">
              A Platform Built{" "}
              <span className="bg-gradient-to-r from-emerald-500 to-emerald-700 bg-clip-text text-transparent">with Love</span>
              {" "}for the Ummah
            </motion.h2>
            <motion.p variants={item} className="text-emerald-700/80 text-lg leading-relaxed mb-6">
              Zahoor is a comprehensive Islamic social platform designed specifically for the global Shia Muslim community. We combine spiritual tools, community services, and educational resources into one beautifully crafted experience.
            </motion.p>
            <motion.p variants={item} className="text-emerald-700/70 leading-relaxed mb-8">
              From Prayer Times and Qibla direction to Ejara services, Istikhara, and live Majalis — Zahoor is your complete spiritual companion.
            </motion.p>
            <motion.div variants={item} className="flex flex-wrap gap-3">
              {["🕌 Spiritual Tools", "🤝 Community First", "🌐 Global Reach", "🔒 Safe & Secure"].map((tag) => (
                <span key={tag} className="px-4 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-medium">{tag}</span>
              ))}
            </motion.div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 60 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, ease: "easeOut", delay: 0.2 }} className="relative">
            <div className="relative bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-3xl p-8 text-white shadow-2xl shadow-emerald-200/50 overflow-hidden">
              <svg className="absolute inset-0 w-full h-full opacity-10" xmlns="http://www.w3.org/2000/svg">
                <defs><pattern id="card-p" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
                  <path d="M20 0 L40 20 L20 40 L0 20 Z" fill="none" stroke="white" strokeWidth="0.5"/>
                </pattern></defs>
                <rect width="100%" height="100%" fill="url(#card-p)"/>
              </svg>
              <div className="relative z-10">
                <div className="text-5xl mb-4">🌿</div>
                <h3 className="text-2xl font-bold mb-3 text-white">Our Mission</h3>
                <p className="text-emerald-100 leading-relaxed">
                  To empower every believer with access to authentic Islamic resources, meaningful community connections, and trusted spiritual services — anytime, anywhere.
                </p>
              </div>
            </div>
