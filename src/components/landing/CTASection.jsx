    ...
    <section ref={ref} className="py-24 relative overflow-hidden bg-gradient-to-br from-[#f0fdf6] to-white">
      ...
      <svg className="absolute inset-0 w-full h-full opacity-[0.03]" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="cta-pat" x="0" y="0" width="60" height="60" patternUnits="userSpaceOnUse">
            <path d="M30 0 L60 30 L30 60 L0 30 Z" fill="none" stroke="#059669" strokeWidth="0.5"/>
          </pattern>
        </defs>
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
        ...
        >
          ...
          >
            <span>🌿</span> Join 50,000+ Believers
          </motion.div>
          ...
          <p className="text-emerald-700/80 text-xl leading-relaxed mb-12 max-w-2xl mx-auto">
            Join Zahoor for free and experience the most comprehensive Islamic community platform ever built. 
            Your faith, your community, your Zahoor.
          </p>

          {/* CTA Buttons */}
          <div className="flex flex-col sm:flex-row gap-5 justify-center">
            <motion.button
              whileHover={{ scale: 1.07, boxShadow: "0 0 40px rgba(16,185,129,0.5)" }}
              whileTap={{ scale: 0.97 }}
              onClick={() => base44.auth.redirectToLogin()}
              className="px-10 py-5 rounded-2xl text-xl font-black text-white bg-gradient-to-r from-emerald-500 to-emerald-600 shadow-2xl shadow-emerald-200 transition-all duration-200"
            >
              🌿 Create Free Account
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.07, boxShadow: "0 0 30px rgba(245,158,11,0.4)" }}
              whileTap={{ scale: 0.97 }}
              onClick={() => base44.auth.redirectToLogin()}
              className="px-10 py-5 rounded-2xl text-xl font-black text-amber-700 bg-gradient-to-r from-amber-50 to-yellow-50 border-2 border-amber-300 shadow-xl shadow-amber-100 transition-all duration-200"
            >
              ✨ Sign In
            </motion.button>
          </div>
          ...
          >
            {[
              ...
              <motion.div
                key={i}
                whileHover={{ y: -5 }}
                className="bg-white/80 backdrop-blur-sm border border-emerald-100 rounded-2xl p-6 shadow-lg text-left"
              >
