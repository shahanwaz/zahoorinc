  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-white/90 backdrop-blur-xl shadow-lg shadow-emerald-100/50 border-b border-emerald-100"
          : "bg-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => base44.auth.redirectToLogin()}
            className="px-5 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-emerald-600 shadow-lg shadow-emerald-200 transition-all duration-200"
          >
            Get Started
          </motion.button>
          ...
          {/* Mobile Hamburger */}
          <button
            className="md:hidden ml-2 text-emerald-700"
            ...
            <div className="px-6 py-4 space-y-3">
              {["features", "spiritual", "how-it-works", "about"].map((id) => (
                <button
                  key={id}
                  onClick={() => scrollTo(id)}
                  className="block text-sm font-medium text-emerald-700 py-1 capitalize"
                >
                  {id.replace("-", " ")}
                </button>
              ))}
